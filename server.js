...
app.post("/api/notes", (req, res) => {
    let newNote = req.body; //a new note is made from the text in the request body and assigned to a variable
    let noteList = JSON.parse(fs.readFileSync("./db/db.json", "utf8")); //make a variable of all the notes parsed. This is so we can add newNote later
    let notelength = (noteList.length).toString(); //we check how many notes we have in the list. This allows us to add id's to the notes
...
app.delete("/api/notes/:id", (req, res) => { //we target a specific note based on the id
    let noteList = JSON.parse(fs.readFileSync("./db/db.json", "utf8")); //once again we make a variable that is a list of all the saved notes
    let noteId = (req.params.id).toString(); //we turn the requested id into a string. This is because the note id's are all strings
